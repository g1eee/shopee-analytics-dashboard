...
export async function parseProdukXlsx(file: File): Promise<ParsedFile> {
  ...
  const C = {
    ...
    halamanProdukDilihat: colIdx('Halaman Produk Dilihat'),
    produkDiklik: colIdx('Produk Diklik'),
    klikPencarian: colIdx('Klik Pencarian'),
    ctr: colIdx('Persentase Klik'),
  ...
  for (let i = 1; i < arr.length; i++) {
    ...
    produk.push({
      ...
      halamanProdukDilihat: parseIdNum(r[C.halamanProdukDilihat]),
      produkDiklik: parseIdNum(r[C.produkDiklik]),
      klikPencarian: parseIdNum(r[C.klikPencarian]),
